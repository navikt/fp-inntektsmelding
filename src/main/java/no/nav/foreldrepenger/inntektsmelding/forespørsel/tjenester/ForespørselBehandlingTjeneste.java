...
public class ForespørselBehandlingTjeneste {
    ...
    private void opprettForespørselMinSideArbeidsgiver(UUID forespørselUuid, OrganisasjonsnummerDto orgnummer, AktørId aktørId, Ytelsetype ytelsetype,
                                                       LocalDate førsteUttaksdato) {
        var organisasjon = organisasjonTjeneste.finnOrganisasjon(orgnummer.orgnr());

        var person = personTjeneste.hentPersonInfoFraAktørId(new no.nav.foreldrepenger.inntektsmelding.integrasjoner.person.AktørId(aktørId.getAktørId()), ytelsetype);
    ...
    public NyBeskjedResultat opprettNyBeskjedMedEksternVarsling(SaksnummerDto fagsakSaksnummer,
        ...
        var person = personTjeneste.hentPersonInfoFraAktørId(new no.nav.foreldrepenger.inntektsmelding.integrasjoner.person.AktørId(forespørsel.getAktørId().getAktørId()), forespørsel.getYtelseType());
    ...
    private void validerAktør(ForespørselEntitet forespørsel, AktørId aktorId) {
    ...
    public List<ForespørselEntitet> hentForespørsler(OrganisasjonsnummerDto orgnr,
        ...
        return forespørselTjeneste.hentForespørsler(orgnr,
            Optional.ofNullable(aktørId).map(no.nav.foreldrepenger.inntektsmelding.integrasjoner.person.AktørId::getAktørId).map(AktørId::new).orElse(null),
