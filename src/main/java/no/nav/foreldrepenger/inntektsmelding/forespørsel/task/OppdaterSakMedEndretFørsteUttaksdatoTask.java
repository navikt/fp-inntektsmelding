...
public class OppdaterSakMedEndretFørsteUttaksdatoTask implements ProsessTaskHandler {
    ...
    public void doTask(ProsessTaskData prosessTaskData) {
        prosessTaskData.validerProperties(Set.of(FellesTaskProperties.KEY_FOERSTE_UTTAKSDATO));
        var førsteUttaksdato = LocalDate.parse(prosessTaskData.getPropertyValue(FellesTaskProperties.KEY_FOERSTE_UTTAKSDATO));
        ...
        if (!førsteUttaksdato.equals(forespørsel.førsteUttaksdato())) {
            LOG.info("Første uttaksdato er endret igjen for forespørsel {}, oppdaterer ikke sak", forespørselUuid);
            return;
        }
        ...
        minSideArbeidsgiverTjeneste.sendBeskjedOmEndretFørsteUttaksdato(forespørsel, sisteEndring.førsteUttaksdato(), førsteUttaksdato);
